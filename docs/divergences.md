...
## 9. `opcli env deploy-registry` — local OCI registry management
...
**Implementation:** `opcli env deploy-registry` is added as a new subcommand
under `opcli env`. It auto-detects the active k8s provider by checking
which binary is on PATH (priority: `microk8s` → `k8s` → `kubectl`) and
deploys a local OCI registry at `localhost:32000`:

- **MicroK8s detected**: uses `sudo microk8s kubectl` to apply an embedded
  `registry:2` Deployment + NodePort 32000 Service manifest, then waits for
  the deployment to roll out.
- **Canonical k8s detected**: uses `sudo k8s kubectl` with the same manifest.
- **Standalone kubectl detected**: uses `sudo kubectl` as a fallback.
- **Nothing on PATH**: no-op (skipped with a log message).
...
calls `opcli env deploy-registry` after `concierge prepare`,
...
endpoint that matches the default `opcli artifacts push-images` target.
...
## 13. `opcli artifacts push-images` writes back rock image refs to `artifacts.build.yaml`

**Spec:** `opcli artifacts push-images` loads rock images into the local registry.
...
`opcli artifacts push-images` updates the corresponding arch build entry's `image`
field in `artifacts.build.yaml` with the pushed image reference (e.g.
`localhost:32000/myrock:amd64`).

This means that after running `opcli artifacts push-images`, `opcli pytest expand`
...
- `opcli artifacts push-images` — updates only `rock.builds.image`; no charm resource
  fields to update.
...
## 26. `opcli install spread` / `opcli install tox` / `opcli install concierge` / `opcli env provision` — extracted subcommands
...
| `opcli env provision -c <file>` | Runs `concierge prepare` to provision the test environment. Raises if the config file does not exist. |
...
Additionally, `opcli artifacts push-images` is now a safe no-op when
