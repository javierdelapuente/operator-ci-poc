...
# creates spread.yaml and tests/integration/run/task.yaml.
opcli spread init
...
4. **Test (matrix).** Each job runs a single spread test with the  ci:  backend, which targets the current GitHub runner. Inside each job:  
   1. Backend prepare  
      ...
      4. The job waits for the build collection step and downloads  artifacts.build.yaml  
...
| artifacts.build.yaml | No. Generated by the build step. Should be in .gitignore. | Extends the artifacts.yaml with the paths or references of the build artifacts. |
...
By default this file will be placed under tests/integration/run and auto-generated by “opcli spread init”. 
...
| opcli artifacts build | Builds all artifacts declared in artifacts.yaml and produces artifacts.build.yaml | --charm \<name\>: build only this charm (repeatable). --rock \<name\>: rocks to build (repeatable). --snap \<name\>: snaps to build (repeatable). |
